...
- **`fileState` coordinates the on-chain renewal worker.** New uploads are created `"to-extend"` — a
  fresh blob's initial on-chain Walrus lifetime still needs extending, so the worker picks these up
  too. When a **renew payment settles**, `extendFileRecordExpiry` (in the `onAfterSettle` hook)
  atomically sets `fileState: "renew"` alongside the new `expiresAt`. Both `"to-extend"` and `"renew"`
  are signals the out-of-band renewal service uses to find blobs whose on-chain lifetime needs
  extending. Flipping the record to `"active"` after the on-chain extension is **owned by that separate
  microservice, not this API**. Upload settlement (`markFileRecordPaid`) leaves `fileState` untouched
  since the record was already created `"to-extend"`.
...
x402/
  src/
    ...
    db/
      ...
      fileRecord.ts       # putFileRecord, getFileRecordById, markFileRecordPaid,
                          #   extendFileRecordExpiry, getFileRecordsByPublicKey
    utils/
...
| `fileState`     | `"to-extend"` \| `"renew"` \| `"active"`; renewal-worker flag |
